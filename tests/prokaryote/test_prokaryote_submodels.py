...
class ProkaryoteTestCase(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.kb = wc_kb.io.Reader().run('tests/fixtures/core.xlsx',
    ...
    def test_compartments_generator(self):
        self.assertIsInstance(self.model.compartments.get_one(
            id='c'), wc_lang.core.Compartment)
        self.assertIsInstance(self.model.compartments.get_one(
            id='e'), wc_lang.core.Compartment)
    ...
    def test_metabolites_generator(self):
        ...
        specie = self.model.species_types.get_one(
            id='ATP').species.get_one(compartment=comp)
        ...
        comp = self.model.compartments.get_one(id='c')
        specie = self.model.species_types.get_one(
            id='H2O').species.get_one(compartment=comp)
        self.assertIsInstance(specie, wc_lang.core.Species)
...
class TranslationSubmodelTestCase(ProkaryoteTestCase):
    ...
    def test_construction(self):
        self.assertEqual(
            len(self.model.get_species_types(
